...
export async function generateMetadata({
  ...
  return {
    title,
    description: `An app generated on Sitecoder: ${title}`,
    openGraph: {
    ...
    <div className="flex h-full w-full grow flex-col">
      ...
      <div className="fixed bottom-4 right-4 z-50 hidden md:block">
        <a
          ...
          href={`https://sitecoder-main-moidev-158cafc1.koyeb.app/?ref=${messageId}`}
          target="_blank"
          rel="noopener noreferrer"
        >
          <span className="text-center">
            Powered by <span className="font-semibold">Sitecoder</span>
          </span>
        </a>
      </div>
    </div>
...
const getMessage = cache(async (messageId: string) => {
  return getCachedMessageWithChat(messageId);
});
